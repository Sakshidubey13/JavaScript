...
// WAP to check whether a number is Magic Number or not using while loop, do…while loop, for loop.
    let magicNum = 1729;
    let originalMagicNum = magicNum;
    let sumOfDigits = 0;        


    console.log("Using while loop : "); 
    while (originalMagicNum != 0) {
        let digit = originalMagicNum % 10;
        sumOfDigits += digit;
        originalMagicNum = Math.floor(originalMagicNum / 10);
    }       
    if (sumOfDigits === 1) {
        console.log(magicNum + " is a Magic number.");
    } else {
        console.log(magicNum + " is not a Magic number.");
    }  
    
    

    console.log("Using do while loop : ");
    originalMagicNum = magicNum;
    sumOfDigits = 0;    
    do {    
        let digit = originalMagicNum % 10;
        sumOfDigits += digit;
    ...
    originalMagicNum = magicNum;
    sumOfDigits = 0;    
    for(let originalMagicNum=magicNum; originalMagicNum !=0; ) {    
        let digit = originalMagicNum % 10;
        sumOfDigits += digit;
        originalMagicNum = Math.floor(originalMagicNum / 10);
    }   
